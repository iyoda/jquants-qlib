...
2. On GitHub, create the environment `pypi` (Settings → Environments) and restrict its
   deployment branches/tags to `v*`. Add required reviewers if you want a manual approval gate.

If a tag is pushed with `PUBLISH_TO_PYPI` set but the setup incomplete, fix the setup and re-run
the failed jobs of the same workflow run; do not retag.
...
2. In [CHANGELOG](../CHANGELOG.md), rename the `## [Unreleased]` heading to `## [X.Y.Z] - YYYY-MM-DD`
   ...
   yet, so the first tag `v0.1.0` turns the current `Unreleased` section into `[0.1.0] - <date>`.
   The release workflow extracts exactly the `## [X.Y.Z]` section as the GitHub Release notes and
   **fails** if that heading is missing, so this step is mandatory. Include compatibility and
...
5. Merge the reviewed version and changelog changes and confirm all CI jobs pass on that commit.
   Create and push `vX.Y.Z` on that commit:
...
6. The release workflow checks the tag against installed package metadata, extracts the
   `## [X.Y.Z]` CHANGELOG section into the release notes (failing early if it is missing), runs
   ...
   otherwise it is skipped. The final job creates the GitHub Release from the extracted notes and
   attaches the same files. Inspect every destination you use. A GitHub Release failure after
   PyPI succeeds does not undo the PyPI upload; rerun only failed jobs. Never replace an already
   published version.
...
| --- | --- | --- |
| Package `X.Y.Z` | Every distribution release; patch for compatible fixes, minor for new features. During `0.x`, breaking CLI changes require a minor bump and explicit notes; from `1.0`, use a major bump. | Packaging/docs changes alone do not require regeneration. State the effect of behavioral fixes. |
